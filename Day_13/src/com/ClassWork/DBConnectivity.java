	...
	public static void main(String[] args)
	{
		...
		//To update the data
			...
			*/
			...
			//To update diff record like name,age ,disease
			
			String Name = "UPDATE patient SET name = ? WHERE id = ?";
			String Age = "UPDATE patient SET age = ? WHERE id = ?";
			String Disease = "UPDATE patient SET disease = ? WHERE id = ?";
			
			// Update name
			PreparedStatement pstmtName = con.prepareStatement(Name);
			pstmtName.setString(1, "Anshi");
			pstmtName.setInt(2,101);  // assuming ID=101
			pstmtName.executeUpdate();

			// Update age
			PreparedStatement pstmtAge = con.prepareStatement(Age);
			pstmtAge.setInt(1,24);
			pstmtAge.setInt( 2,102); // ID=102
			pstmtAge.executeUpdate();

			// Update disease
			PreparedStatement pstmtDisease = con.prepareStatement(Disease);
			pstmtDisease.setString(1, "Flu");
			pstmtDisease.setInt(2,103); // ID=103
		...
		//To update multiple names at a time.
